...
const FrontendApp = () => {
      ...
      try {
        ...
        setData((prevData) => {
          ...
          setSuccessRate((prevSuccessRate) => {
            ...
            const successfulDataCount = updatedData.filter((item) => item.success).length;
  ...
  {data.map((item, index) => (
    <li key={index}>
      ...
      Name: {item.name}, Origin: {item.origin}, Destination: {item.destination}, Secret Key: {item.secret_key}
    </li>
  ))}
</ul>
